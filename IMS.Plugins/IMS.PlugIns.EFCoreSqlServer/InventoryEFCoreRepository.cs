...
namespace IMS.Plugins.EFCoreSqlServer
{
    public class InventoryEFCoreRepository : IInventoryRepository
    {
        ...
        {
            using var db = this.contextFactory.CreateDbContext();
            return (await db.Inventories.Where(x => x.InventoryName.ToLower().IndexOf(inventory.InventoryName.ToLower()) >= 0).ToListAsync()).Count > 0;
        ...
        {
            using var db = this.contextFactory.CreateDbContext();
            return await db.Inventories.Where(x =>
